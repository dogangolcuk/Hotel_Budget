...
const ButceOlusturPersonel = () => {
  ...
  const Hesapla = () => {
    getDataByRoute2('/forecastpersonel')
      .then((result) => {
        let kum_gelir = 0
        let kum_gider = 0
        let kum_balance = 0
        ...
        const butce_data = result.data.map((row) => {
          if (
            ...
            let gider = row.gunluk_maliyet * butceperiod_gun_sayisi
  ...
  return (
    <>
      ...
      >
        <CTableCaption>
          <b style={{ color: stc('Personel Kısmı Bütçesi') }}>Personel Kısmı Bütçesi</b>{' '}
...
export default ButceOlusturPersonel
