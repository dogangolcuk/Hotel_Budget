...
const ButceOlusturOda = () => {
      ...
      .then((result) => {
        ...
        const butce_data = result.data.map((row) => {
          if (
            ...
            let butceperiod_gun_sayisi = differenceInDays(
              new Date(row.butceperiod.bitis_tarihi),
              new Date(row.butceperiod.baslama_tarihi),
            )
            butceperiod_gun_sayisi = butceperiod_gun_sayisi + 1
            let gelir = row.doluluk_oran * row.gelir_miktar * row.oda.adet * butceperiod_gun_sayisi
            let enflasyon_düzeltmesi_gelir = gelir * row.butceperiod.tahmini_enflasyon
            let gider = row.doluluk_oran * row.gider_miktar * row.oda.adet * butceperiod_gun_sayisi
  ...
  return (
    <>
      ...
      >
        <CTableCaption>
          <b style={{ color: stc('Oda Kısmı Bütçesi') }}>Oda Kısmı Bütçesi</b>{' '}
...
export default ButceOlusturOda
