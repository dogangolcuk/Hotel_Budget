...
const ButceOlusturDiger = () => {
  ...
  const Hesapla = () => {
    getDataByRoute2('/forecastdigergelirgider')
      .then((result) => {
        let kum_gelir = 0
        let kum_gider = 0
        let kum_balance = 0
        ...
        const butce_data = result.data.map((row) => {
          if (
            ...
            return {
              ad: 'DİĞER_GELİR_GİDER#' + row.id + '#' + row.ad,
  ...
  return (
    <>
      ...
      >
        <CTableCaption>
          <b style={{ color: stc('Diğer Gelir/Gider Bütçesi') }}>Diğer Gelir/Gider Bütçesi</b>{' '}
...
export default ButceOlusturDiger
